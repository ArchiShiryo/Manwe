...
Contrôle navigateur : `node scripts/ui-check-lieu.mjs`. **D-032, agent qui cartographie** : objectif de départ modifiable (`/api/agent/mission`), plan de couverture et profil cité (`/api/agent/coverage`, migration 016), affichés dans le lieu ; l'agent de conversation choisit ses questions dans les creux du plan. **Lanceur Windows** : `MANWE.cmd` à la racine (double-clic) met à jour depuis git, installe si besoin, demande la clé DeepSeek (mémorisable, chiffrée pour le compte Windows, hors du dépôt) et ouvre l'interface ; options `-Espace nom`, `-SansMaj`, `-OublierCle`. Corrections après la seconde démonstration (DEMO-R5-7, constats 25 à 28) : extraction avant interprétation, analyses appliquées malgré les notes écrites pendant le calcul, opération refusée écartée seule, conversation capable de lancer la modélisation.
