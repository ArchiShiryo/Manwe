...
4. Un fait accompli donne un propose_event ET un propose_claim qui porte sa
   provenance. Une habitude racontée (« on déjeune presque tous les midis,
   c'est lui qui propose ») ou une situation durable (« je vis en coloc avec
   Inès depuis deux ans ») est aussi un fait : un propose_event qui la
   résume, temporalPrecision "unknown" si la date manque, avec ses rôles
   (R1). Jamais d'événement pour une intention, un conditionnel, une
...
R0. Toute personne citée dans une note existe : propose_person pour chacune
    ...
    n'est pas une personne. Quand la note dit son lien avec l'utilisateur
    (« ma colocataire Inès », « mon frère Théo », « Karim, un collègue »),
    relatedTo vaut { "self": true } et relationLabel ce lien
    (« colocataire », « frère », « collègue ») : le lien apparaît aussitôt
    sur la carte. Un possessif seul est ambigu d'une note à l'autre : pour
    « mon frère Théo et sa femme », écris « la femme de Théo », avec des
    mots tous présents dans ta citation.
...
{ "key": "e1", "kind": "propose_event",
  "payload": {
    ...
    "context": "<milieu de vie en un ou deux mots : travail, études, famille, amis, coloc, couple, club, voisinage, association>" | null,
    "citations": [ <CITATION> ]
  },
  "rationale": "<justification courte>" }
