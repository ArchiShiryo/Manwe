...
- [x] R5.1 Réaliser le parcours de trois personnes et vingt événements depuis un espace vide. (Preuve, 25 septembre : lot R5-S05, deux parcours de vingt notes, aucune analyse perdue, 9/9 [L], 8/8 [B] ([RAPPORT-014](./manwe-next/docs/pilotage/rapports/RAPPORT-014.md)) ; historique des lots non validés : [RAPPORT-011](./manwe-next/docs/pilotage/rapports/RAPPORT-011.md), [RAPPORT-012](./manwe-next/docs/pilotage/rapports/RAPPORT-012.md), [RAPPORT-013](./manwe-next/docs/pilotage/rapports/RAPPORT-013.md).)
...
- [x] R5.3 Permettre la correction sur une preuve, la réponse à une question et l'inspection de la révision produite. (Preuve, 25 septembre : correction d'une preuve depuis le graphe et l'inspecteur (`scripts/ui-check-r4.mjs`) ; réponse à une question qui met les lectures visées à réexaminer, puis révision visible dans leur historique (`scripts/ui-check-r5-3.mjs`) ; libellés d'historique pour les choix et résultats d'action ; l'interface vouvoie partout.)
- [x] R5.4 Faire émerger un problème et un objectif de la conversation ; rendre leur formulation modifiable immédiatement. (Preuve, 25 septembre : opération `propose_goal` (contrat 1.6), proposition non confirmée tant que l'utilisateur ne l'adopte pas, reformulation et écart sur l'écran Intentions (`scripts/ui-check-goal.mjs`) ; objectif proposé, cité et adopté sur les deux scénarios de R5-S05 ([RAPPORT-014](./manwe-next/docs/pilotage/rapports/RAPPORT-014.md)).)
...
- [x] R5.5b Boucler l'action : action proposée validée par l'utilisateur, attente enregistrée avant l'essai, résultat observé, comparaison entre prédiction et réalité, puis révision des hypothèses concernées (D-016). (Preuve : routes `POST /api/actions` et `/api/actions/:id/outcome`, attente figée datée avant le résultat (contrainte en base), résultat en annotation citable sur la lecture actionnée, paquet de réanalyse avec prédictions et résultat ; D01 de R5-S01 : la réanalyse compare explicitement prédictions et résultat ([RAPPORT-010](./manwe-next/docs/pilotage/rapports/RAPPORT-010.md)) ; interface `Directions.tsx` sur l'écran Intentions (choix avec attente propre, résultat, verdicts facultatifs, réanalyse), vérifiée par `scripts/ui-check-directions.mjs`.)
- [x] R5.6 Rejouer le parcours avec un autre groupe et d'autres noms, sans modifier le code, le contrat ni le prompt pour le scénario. Sol produit de nouvelles propositions à partir du seul paquet de contexte autorisé ; ne pas recopier les réponses attendues du corpus. (Preuve, 25 septembre : P02 rejoue P01 avec un autre groupe, d'autres noms et d'autres mots, même code, même contrat, même prompt ; lot R5-S05 validé ([RAPPORT-014](./manwe-next/docs/pilotage/rapports/RAPPORT-014.md)).)
- [ ] R5.7 Effectuer une démonstration utilisateur et noter les incompréhensions ainsi que les corrections nécessaires. (En cours, 25 septembre : première démonstration interrompue après l'extraction ([DEMO-R5-7](./manwe-next/docs/pilotage/rapports/DEMO-R5-7.md)) ; le parcours est remplacé par un lieu (D-029, [BRIEF-007](./manwe-next/docs/pilotage/briefs/BRIEF-007-le-lieu.md)) ; démonstration à rejouer dans le lieu.)
- [ ] R5.8 Entretien guidé : l'agent voit toute la mémoire, repère les creux (milieux, personnes, périodes, lectures sans preuve) et pose une question à la fois dans la conversation ; l'utilisateur peut aussi écrire librement ou tenir son journal ; l'agent analyse seul, sans confirmation (D-028, D-029). Toute personne citée existe, même décrite sans nom, et son identité se met à jour rétrospectivement (D-030). (En cours, 25 septembre : conversation, agent autonome, personnes décrites et renommables, écran du lieu codés et vérifiés en navigateur avec un fournisseur simulé (`scripts/ui-check-lieu.mjs`) ; puis, après la seconde démonstration, extraction avant interprétation, analyses appliquées malgré les notes écrites pendant le calcul, et agent qui cartographie (D-032 : objectif de départ, plan de couverture pour chaque acteur, profil cité) ; restent la fusion de deux fiches, un lot scellé sur DeepSeek et la démonstration à l'utilisateur.)
